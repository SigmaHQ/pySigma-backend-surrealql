...
class SurrealQLBackend(TextQueryBackend):
    ...
    field_quote_pattern: ClassVar[Pattern] = re.compile("^[a-zA-Z0-9_]*$")  # Pattern to match field names that don't need quoting
    field_quote_pattern_negation: ClassVar[bool] = True                     # If True, the pattern matches field names that need quoting
    ...
    wildcard_match_expression: ClassVar[str] = "string::matches({field},{value})"   # Special expression if wildcards can't be matched with the eq_token operator
    ...
    field_exists_expression : ClassVar[str] = "{field} IS NOT NONE"             # Expression for field existence as format string with {field} placeholder for field name
    field_not_exists_expression : ClassVar[str] = "{field} IS NONE"      # Expression for field non-existence as format string with {field} placeholder for field name. If not set, field_exists_expression is negated with boolean NOT.
    ...
    and_in_operator : ClassVar[str] = "CONTAINSALL"   # Operator used to convert AND into in-expressions. Must be set if convert_and_as_in is set
